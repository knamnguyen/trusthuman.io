...
window.engagekitInternals = {
  ...
  async expandPostComments(postContainer) {
    const beforeCommentsLength = this.extractPostComments(postContainer).length;
    ...
    const waitForCommentsReady = () => {
      return this.retry(
        () => {
          const commentsLength = this.extractPostComments(postContainer).length;
