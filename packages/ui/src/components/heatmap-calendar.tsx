...
export type HeatmapCalendarProps = {
  title?: string;
  data: HeatmapDatum[];
  /** Number of days to show (used if startDate not provided) */
  rangeDays?: number;
  /** Start date for the heatmap range */
  startDate?: Date;
  /** End date for the heatmap range (defaults to today) */
  endDate?: Date;
  weekStartsOn?: 0 | 1;
  cellSize?: number;
  cellGap?: number;
  ...
  /** Portal container for tooltips (useful in Shadow DOM) */
  tooltipContainer?: HTMLElement | null;
  className?: string;
  /** Disable card wrapper for embedding in other cards */
  disableCard?: boolean;
  /** Reverse column order so most recent is on left */
...
export function HeatmapCalendar({
  ...
  // If startDate is provided, use it; otherwise calculate from rangeDays
  const start = startDate ? startOfDay(startDate) : addDays(end, -(rangeDays - 1));
  ...
  const columnsRaw: HeatmapCell[][] = [];
  for (let i = 0; i < weeks; i++) {
    columnsRaw.push(cells.slice(i * 7, i * 7 + 7));
  }

  // Apply reverse order if requested (most recent on left)
  const columns = reverseOrder ? [...columnsRaw].reverse() : columnsRaw;
    ...
    <div className={cn("min-w-35", legendCfg.className)}>
      {showText ? (
        <div className="text-muted-foreground mb-2 text-xs">
          {lessText} {showArrow ? <span aria-hidden>&rarr;</span> : null}{" "}
          {moreText}
        </div>
      ) : null}
  ...
  const tooltipNode = (cell: HeatmapCell) => {
    ...
    const unit = cell.value === 1 ? "verification" : "verifications";
    return (
