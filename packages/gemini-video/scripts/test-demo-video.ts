...
async function main() {
  ...
  try {
    ...
    console.log('💡 Convert start/end times to MM:SS format for VideoStitch ranges.');
    ...
    result.segments.forEach((segment, index) => {
      ...
      console.log(`${index + 1}. { range: "${range}", caption: "${segment.caption}" }`);
    });
    ...
    console.error('   - Try reducing maxDuration or numSegments if the video is too short');
