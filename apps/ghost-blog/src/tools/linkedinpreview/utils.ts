...
function buildUnderlineIgnore() {
    const repeated = UNDERLINE_IGNORE.reduce((total, char) => {
        const permutations = Object.keys(TRANSFORMS).map((key) => applyTransform(char, TRANSFORMS[key as Transforms]))
...
export function processNodes(node: Node): Node {
    ...
    if (node.type === 'text') {
        const mappedMarks = (node.marks || []).reduce((acc, mark) => [...acc, mapMarks(mark.type)], [] as string[])
...
export function applyStyles(characters: string, style: string[]): string {
    ...
    const styledText = transforms.reduce((text, transform) => applyTransform(text, transform), characters)
    return appenders.reduce((text, appender) => applyAppender(text, appender), styledText)
}
...
function retrieveTransforms(styles: Transforms[]): Array<Transform> {
    ...
    if (combined === 'BOLDITALIC' && COMBINED_TRANSFORMS[combined as keyof typeof COMBINED_TRANSFORMS]) {
        return [COMBINED_TRANSFORMS[combined as keyof typeof COMBINED_TRANSFORMS] as Transform]
    }

    return styles.map((s) => TRANSFORMS[s]).filter((t) => t) as Array<Transform>
}
...
function retrieveAppenders(styles: string[]): Array<Appender> {
    return styles.map((style) => APPENDERS[style as keyof typeof APPENDERS]).filter((a) => a) as Array<Appender>
}
