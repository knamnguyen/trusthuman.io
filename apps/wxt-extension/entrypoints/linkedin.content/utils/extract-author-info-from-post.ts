 ...
 *
 * Uses the same reliable extraction patterns as extract-profile-info.ts:
 * - Find profile image via alt text pattern
 * - Navigate from image to anchor for profile URL
 * - Extract name from alt text
 * - Find headline via sibling navigation (not heuristic text matching)
 */
 ...
 *
 * We find the headline by:
 * 1. Starting from the photo anchor
 * 2. Finding the next anchor sibling (name link) or going to parent and finding headline there
 * 3. Looking at children/siblings for headline text
 */
 ...
 */
export function extractAuthorInfoFromPost(
