...
function extractTextWithLineBreaks(element: HTMLElement): string {
  ...
  function processNode(node: Node) {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      const isBlock = ["div", "p", "li", "h1", "h2", "h3", "h4", "h5", "h6"].includes(tagName);
...
/**
 * Get first N words from caption as preview
 */
export function getCaptionPreview(caption: string, wordCount = 10): string {
