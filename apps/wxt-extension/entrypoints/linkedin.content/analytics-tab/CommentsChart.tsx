...
import type { CommentsData } from "../utils/data-fetch-mimic/dashboard-activity-collectors";
  ...
  const chartData: ChartDataPoint[] = [...snapshots]
    ...
    .map((snapshot, index, arr) => {
      ...
      const changeSinceLastFetch = nextSnapshot
        ? snapshot.data.totalComments - nextSnapshot.data.totalComments
        : 0;
      ...
      return {
        ...
        totalComments: snapshot.data.totalComments,
    ...
    return (
      <div className="bg-background rounded-lg border p-3 shadow-md">
        ...
        <div className="text-muted-foreground space-y-1 text-xs">
          <div>
            {data.change > 0 ? "+" : ""}
            {data.change.toLocaleString()} comments since{" "}
            {formatTimeAgo(
              data.timestamp - data.daysElapsed * 24 * 60 * 60 * 1000,
            )}
          </div>
          <div>~{Math.abs(data.dailyRate).toFixed(1)} comments/day avg</div>
        </div>
      </div>
    );
  };
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Comments over {snapshots.length} snapshots
        </CardDescription>
      </CardHeader>
