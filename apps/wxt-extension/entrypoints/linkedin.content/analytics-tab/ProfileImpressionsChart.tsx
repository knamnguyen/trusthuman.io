...
import type { ProfileImpressionsData } from "../utils/data-fetch-mimic/linkedin-profile-impressions-fetcher";

interface ProfileImpressionsChartProps {
...
export function ProfileImpressionsChart({
  snapshots,
  compact = false,
}: ProfileImpressionsChartProps) {
  ...
  const chartData: ChartDataPoint[] = [...snapshots]
    ...
    .map((snapshot, index, arr) => {
      ...
      const changeSinceLastFetch = nextSnapshot
        ? snapshot.data.totalImpressions - nextSnapshot.data.totalImpressions
        : 0;
      ...
      return {
        ...
        totalImpressions: snapshot.data.totalImpressions,
    ...
    return (
      <div className="bg-background rounded-lg border p-3 shadow-md">
        ...
        <div className="text-muted-foreground space-y-1 text-xs">
          <div>
            {data.change > 0 ? "+" : ""}
            {data.change.toLocaleString()} impressions since{" "}
            {formatTimeAgo(
              data.timestamp - data.daysElapsed * 24 * 60 * 60 * 1000,
            )}
          </div>
          <div>
            ~{Math.abs(data.dailyRate).toFixed(1)} impressions/day avg
          </div>
        </div>
      </div>
    );
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Profile impressions over {snapshots.length} snapshots
        </CardDescription>
      </CardHeader>
