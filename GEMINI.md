...
This is a monorepo for the EngageKit application, built with the T3 Turbo Stack. It includes a Next.js web application, a Chrome extension, and several shared packages. The project uses pnpm as a package manager, Turborepo for monorepo management, tRPC for API communication, and Tailwind CSS for styling. For the database, it utilizes Prisma with a PostgreSQL database.
...
To run all applications and packages in development mode, use the following command:

```bash
pnpm dev
```

To run only the Next.js application:
...
To build all applications and packages for production, use the following command:
...
To push database schema changes, use the following command:

```bash
pnpm db:push
```

To open the Prisma Studio, use the following command:
...
*   **Code Style:** The project uses Prettier for code formatting and ESLint for linting. You can run the formatters and linters with the following commands:
    *   `pnpm format`
    *   `pnpm format:fix`
    *   `pnpm lint`
    *   `pnpm lint:fix`
*   **Commits:** Please follow the conventional commit format.
*   **UI Components:** The project uses shadcn/ui for UI components. To add a new component, run `pnpm ui-add`.
*   **Packages:** To add a new package to the monorepo, run `pnpm turbo gen init`.
